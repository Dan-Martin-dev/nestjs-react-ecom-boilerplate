  ...
  async processPayment(order: any, paymentDto: ProcessPaymentDto): Promise<any> {
    try {
      ...
      return {
        ...
        message: 'Payment voucher generated successfully. Please pay at your nearest PagoFacil location.',
        details: {
          provider: 'PagoFacil',
          voucherCode,
      ...
      this.logger.error(`Error processing PagoFacil payment: ${error.message}`, error.stack);
  ...
  async handleWebhook(webhookData: any): Promise<any> {
    try {
      ...
      if (webhookData.action === 'payment_received') {
        ...
        if (!payment) {
          this.logger.warn(`No pending payment found for voucher ${voucherCode}`);
          return { success: false, message: 'No pending payment found for this voucher' };
        }
      ...
      this.logger.error(`Error processing PagoFacil webhook: ${error.message}`, error.stack);
