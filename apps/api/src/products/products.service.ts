  ...
  async findProductByColorSlug(
    ...
    const variantsForColor = product.variants.filter((variant) =>
      variant.ProductVariantAttribute.some(
        (attr) =>
          attr.attribute.name === 'Color' && slugify(attr.value) === colorSlug,
      ),
    );
    ...
    // 5. Construct the response DTO with explicit mapping to match ProductResponseDto
    const mapImage = (img: Image) => ({
      id: img.id,
      url: img.url,
    ...
    const variantsDto = variantsForColor.map((variant) => ({
      id: variant.id,
      name: variant.name,
      slug: variant.slug,
      sku: variant.sku,
      ...
      ProductVariantAttribute: variant.ProductVariantAttribute.map((pva) => ({
        attribute: { name: pva.attribute.name },
        value: pva.value,
        // ProductVariantAttribute does not have a DB relation to ProductAttributeValue
        // (we store the raw `value` on the variant attribute). Provide `attributeValue` as null.
        attributeValue: null,
      })),
    ...
    return {
      ...
      colorSpecificImages: colorSpecificImages.map(mapImage),
